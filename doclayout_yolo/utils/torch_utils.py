...
def model_info_for_loggers(trainer):
    ...
    if trainer.args.profile:  # profile ONNX and TensorRT times
        from doclayout_yolo.utils.benchmarks import ProfileModels
...
def init_seeds(seed=0, deterministic=False):
    ...
    # torch.backends.cudnn.benchmark = True  # AutoBatch problem https://github.com/doclayout_yolo/yolov5/issues/9287
    if deterministic:
        if TORCH_2_0:
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "") -> None:
    ...
    Example:
        ```python
        from pathlib import Path
        from doclayout_yolo.utils.torch_utils import strip_optimizer
