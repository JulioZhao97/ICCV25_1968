...
class YoloSwinReshape(nn.Module):
    def __init__(self):
        super().__init__()
...
class YoloSwinBlock(nn.Module):
    def __init__(self, 
        ...
        for i_layer in range(depths):
            ...
            sd_prob = stochastic_depth_prob * float(stage_block_id[i_layer]) / (total_stage_blocks - 1)
            stage.append(
                block(
                    dim,
                    num_heads[i_stage],
        ...
        if down_sample:
            stage.append(downsample_layer(dim, norm_layer))
        self.features = nn.Sequential(*stage)
